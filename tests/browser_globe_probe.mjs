...
const targets=[
  {name:'candidate-desktop',url:candidate,viewport:{width:1600,height:1000},mobile:false},
  {name:'candidate-mobile',url:candidate,viewport:{width:430,height:932},mobile:true},
];
...
for(const target of targets){
  ...
  await page.route('https://celestrak.org/**',route=>route.fulfill({status:503,contentType:'text/plain',body:'browser regression intentionally tests telemetry-unavailable fallback'}));
  ...
  }catch(err){navError=String(err?.stack||err);hardFailure=true;}
  ...
  await run('boot-contract',()=>page.evaluate(()=>{
    ...
    if(!gl||gl.isContextLost())throw new Error('WebGL context unavailable or lost');
    if(!(renderer.info?.render?.calls>0))throw new Error('renderer has no draw calls');
    ...
    const overflow=document.documentElement.scrollWidth-document.documentElement.clientWidth;if(overflow>2)throw new Error(`horizontal overflow ${overflow}px`);
    return {canvas:{width:rect.width,height:rect.height},drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,counts,dataMode:MODEL_META.dataMode,snapshot:MODEL_META.workbookSnapshotSha256,webgl:gl.getParameter(gl.VERSION)};
  }));
  ...
  await run('side-panel-collapse-expand',async()=>{
    ...
    const mid=await page.locator('#globe-canvas').boundingBox();
    ...
    if(!target.mobile&&!(mid.width>before.width+150))throw new Error(`desktop globe did not expand ${before.width}->${mid.width}`);
    await page.locator('#panel-toggle').click();await page.waitForTimeout(650);
    const after=await page.locator('#globe-canvas').boundingBox();
    ...
    return {before:before.width,collapsed:mid.width,after:after.width};
  });
  ...
  await run('view-switching-object-orbit',async()=>{
    ...
    if(await page.evaluate(()=>S.view)!=='globe')throw new Error('Object view opened without site');
    await page.evaluate(()=>{selectSiteById('LS005');confirmSite();selTransit('LAUNCH_ORBIT');selVehicle('V001');setView('object');});
    await page.waitForTimeout(1000);
    const objectState=await page.evaluate(()=>({view:S.view,renderer:!!lRenderer,scene:!!lScene,width:document.getElementById('launch-canvas')?.getBoundingClientRect().width,height:document.getElementById('launch-canvas')?.getBoundingClientRect().height}));
    if(objectState.view!=='object'||!objectState.renderer||!objectState.scene||objectState.width<200||objectState.height<200)throw new Error('Object view did not initialise');
    await page.evaluate(()=>{setView('orbit');renderOrbitView();});await page.waitForTimeout(250);
    const noData=await page.evaluate(()=>getComputedStyle(document.getElementById('orbit-nodata')).display);if(noData==='none')throw new Error('Orbit no-data state not rendered');
    ...
    const orbitState=await page.evaluate(()=>({view:S.view,noData:getComputedStyle(document.getElementById('orbit-nodata')).display,pressed:document.getElementById('vbtn-orbit').getAttribute('aria-pressed')}));
    if(orbitState.noData!=='none'||orbitState.pressed!=='true')throw new Error('Orbit populated state/view semantics failed');
    await page.evaluate(()=>setView('globe'));
    return {objectState,orbitState};
  });
  ...
  await run('model-phase-and-override-contract',()=>page.evaluate(()=>{
    ...
    return {uprange:[up.CAS_primary,up.CAS_secondary,up.CAS_total],downrangeIgnored:coast.ignoredFailureModes,returnSelected:ret.selectedFailureModes};
  }));
  ...
  if(!target.mobile){
    await run('mission-run-results',async()=>{
      ...
      await page.waitForFunction(()=>S.lastRun&&S.simRunning===false,null,{timeout:12000});await page.waitForTimeout(1100);
      const result=await page.evaluate(()=>({runId:S.lastRun?.runId,total:S.lastRun?.result?.MPL_TOTAL,phase:S.lastRun?.result?.phase,shown:document.getElementById('mpl-results').classList.contains('show'),status:document.getElementById('t-status').textContent,history:RUN_HISTORY.length}));
      if(!result.runId||!(result.total>=0)||!result.shown||result.status!=='MISHAP'||result.history<1)throw new Error('mission run did not reach rendered mishap result');
      return result;
    });

    await run('sweep-evidence-batch-modals',()=>page.evaluate(()=>{
      openSweep();document.getElementById('sw-axis').value='zone';document.getElementById('sw-fm').value='ALL';runSweep();if(sweepResults.length!==5||document.getElementById('sw-export-btn').disabled)throw new Error('zone sweep failed');closeSweep();
      ...
      openBatch();loadBatchPreset('achilles_targets');if(BATCH_QUEUE.length!==WORKBOOK_SNAPSHOT.bridge.length)throw new Error('ACHILLES preset count mismatch');runBatch();if(batchResults.length!==2||batchResults.some(r=>!r.siteId||!r.vehicleId))throw new Error('batch result identity mismatch');closeBatch();
    ...
    await run('acknowledged-write-transport',async()=>{
      ...
      return {writes:writeReceipts.length,tabs:writeReceipts.map(x=>x.tab||x.action||'').filter(Boolean)};
    });
  }else{
    await run('mobile-panel-default',async()=>{
      if(!(await page.evaluate(()=>S.panelCollapsed)))await page.locator('#panel-toggle').click();await page.waitForTimeout(500);
      ...
      if(!state.collapsed||state.expanded!=='false'||state.canvas<state.viewport-4)throw new Error('mobile panel/full-width globe state failed');
      return state;
    });
  }
  ...
  summary.push(record);
  if(pageErrors.length)hardFailure=true;
  await context.close();
}
